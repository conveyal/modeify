...
var _existsSync$writeFileSync$statSync$readdirSync = require('fs');
...
function lookupFiles(path) {
  ...
  if (_existsSync$writeFileSync$statSync$readdirSync.existsSync(path)) {
    var stat = _existsSync$writeFileSync$statSync$readdirSync.statSync(path);
    if (stat.isFile()) {
      return path;
    }
  }
...
function coverCmd(opts) {
  ...
  if (!_existsSync$writeFileSync$statSync$readdirSync.existsSync(cmd)) {
    try {
      cmd = which.sync(cmd);
    } catch (ex) {
  ...
  function prepareCoverage(matchFn) {
    ...
    process.once('exit', function () {
      ...
      _existsSync$writeFileSync$statSync$readdirSync.writeFileSync(file, JSON.stringify(cov), 'utf8');
