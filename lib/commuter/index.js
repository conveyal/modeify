...
import {createAccount, populateAccount, populateAccounts} from '../stormpath'
...
app.get('/', stormpath.authenticationRequired, function (req, res) {
  Commuter
    ...
    .exec()
    .then(commuters => {
      return populateAccounts(req.stormpath, commuters, 'account')
    })
    .then(commuters => {
...
app.post('/', stormpath.authenticationRequired, function (req, res) {
  ...
  createAccount(req.stormpath, req.body.account)
    .then(createdAccount => {
      req.body.commuter.account = createdAccount.id
      return Commuter.create(req.body.commuter)
    })
    .then(createdCommuter => {
      commuter = createdCommuter
      if (req.body.commuter._location) {
...
app.get('/link/:link', function (req, res) {
  Commuter
    ...
    .then(commuter => {
      ...
      return populateAccount(req.stormpath, commuter, 'account')
    })
    .then(commuter => {
