...
app.post('/:id/rideshare-sign-up', auth.commuterIsLoggedIn, get, function(req, res) {
  if (true) return res.status(204).end();

  req.commuter.rideshareSignUp(req.body, function(err) {
    if (err) {
      res.status(400).send(err);
    } else {
      req.session.commuter = req.commuter.toJSON();
