...
schema.statics.generate = function (userData, commuterData) {
...
schema.statics.generateAndSendPlan = function (userData, commuterData, callback) {
  this.generate(userData, commuterData)
    .then((commuter) => {
      commuter.sendPlan(function (err) {
        callback(err, commuter)
      })
    })
    .catch(callback)
}
...
schema.methods.sendEmail = function (template, options) {
  if (this.anonymous && !this._user) {
    return Promise.reject('Cannot send a plan to an anonymous commuter.')
  }

  return new Promise((resolve, reject) => {
    const opts = Object.merge(options, {
      domain: config.domain,
      application: config.application,
...
schema.methods.sendPlan = function (campaign_id, callback) {
  ...
  if (this.anonymous && !this._user) return callback(new Error('Cannot send a plan to an anonymous commuter.'))
