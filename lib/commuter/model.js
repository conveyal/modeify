...
schema.methods.addEmail = function(email, callback) {
  ...
  async.waterfall([
    ...
    function(user, done) {
      if (user) {
        done('Account exists for this email address. Please log in.');
      } else {
    ...
    function(email, done) {
      analytics.track(commuter._user._id, 'Signed Up', {
        type: 'newsletter'
      });
    ...
    this.save(function(err) {
      mandrill.send(emailOptions, function(err, results) {
        if (err) {
          ...
          analytics.track(commuter._user._id, 'Signed Up for Carpool Matching', opts);
...
schema.methods.sendPlan = function(campaign_id, callback) {
  ...
  mandrill.send(options, function(err, results) {
    if (err) {
      ...
      analytics.track(commuter._user._id, 'Sent Commute Plan');
