...
schema.methods.syncWithAccount = function (client) {
  if (!client.account) return Promise.resolve(this)

  return new Promise((resolve, reject) => {
    client.getAccount(`https://api.stormpath.com/v1/accounts/${this.account}`, (err, account) => {
      if (err) {
        reject(err)
      } else {
...
schema.statics.generate = function (stormpath, accountData, commuterData) {
  ...
  createAccount(stormpath, accountData)
    .then(createdAccount => {
      commuterData.account = createdAccount.id
