...
module.exports.requestPlan = function (plan) {
  if (config.r5 || process.env.TEST_R5_URL) {
    ...
    const variables = {
      ...
      maxWalkTime: plan.maxWalkTime,
      maxBikeTime: plan.maxBikeTime
    }
...
function buildQuery ({
  ...
  return `query requestPlan($fromLat: Float!, $fromLon: Float!, $toLat: Float!, $toLon: Float!, $fromTime: ZonedDateTime!, $toTime: ZonedDateTime!, $bikeSpeed:Float!, $walkSpeed:Float!, $maxWalkTime:Int!, $maxBikeTime:Int!) {
    plan(minBikeTime: 1, bikeTrafficStress: ${bikeTrafficStress}, fromLat: $fromLat, fromLon: $fromLon, toLat: $toLat, toLon: $toLon, fromTime: $fromTime, toTime: $toTime, directModes: [${directModes}], accessModes: [${accessModes}], egressModes: [${r5EgressModes}], transitModes: [${r5TransitModes}], bikeSpeed: $bikeSpeed, walkSpeed: $walkSpeed, maxWalkTime: $maxWalkTime, maxBikeTime: $maxBikeTime) {
      patterns {
        tripPatternIdx
        routeId
        routeIdx
