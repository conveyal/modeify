...
app.get('/managers', authenticationRequired, groupsRequired([group.administrator()]), function (req, res) {
  req.stormpathDirectory.getGroups({ name: 'manager', expand: 'accounts' }, (err, groups) => {
    if (err) {
      res.status(400).send(err)
    } else if (groups.items.length < 1) {
      res.status(400).send('Manager group does not exist for this directory.')
    } else {
