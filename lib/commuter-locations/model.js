...
schema.methods.sendProfileAndMatches = function (stormpathClient) {
  return this._commuter.populateAccount(stormpathClient).then((commuter) => {
    return commuter.sendEmail('profile-and-matches', {
...
schema.statics.findCommutersAndProfiles = function (stormpathClient, _location) {
  return new Promise((resolve, reject) => {
    this.find()
      .where('_location', _location)
      .populate('_commuter _location')
      .exec()
      .then(populateCommuters(stormpathClient))
      .then(resolve)
      .catch(reject)
  })
}

function populateCommuters (stormpathClient) {
  return function (cls) {
    return Promise.all(cls.filter(cl => !!cl._commuter).map((cl) => {
      return cl._commuter.populateAccount(stormpathClient).then(() => {
        return cl
      })
    }))
  }
