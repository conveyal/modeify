...
schema.statics.findCommutersAndProfiles = function (stormpathApplication, _location) {
  return new Promise((resolve, reject) => {
    this.find()
      .where('_location', _location)
      .populate('_commuter _location')
      .exec()
      .then(populateCommuters(stormpathApplication))
      .then(resolve)
      .catch(reject)
  })
}

function populateCommuters (stormpathApplication) {
  return function (cls) {
    return Promise.all(cls.filter(cl => !!cl._commuter).map((cl) => {
      return cl._commuter.populateAccount(stormpathApplication).then(() => {
        return cl
      })
    }))
  }
