...
import { profile as otp_profile } from '../otp'
...
function mapCommuterLocationMatches (commuterLocations, matches) {
  return commuterLocations.map(cl => {
    var oldMatchIds = (cl.matches || []).map((match) => { return '' + match._id })
    var newMatches = matches[cl._commuter._id] || []
    var newMatchIds = newMatches.map((match) => { return '' + match._id })
    var diff = newMatchIds.filter((i) => { return oldMatchIds.indexOf(i) < 0 })
