...
import {populateAccount} from '../stormpath'
...
router.get('/', stormpath.authenticationRequired, function (req, res) {
  ...
  if (query._location) {
    CommuterLocations
      .findCommutersAndProfiles(req.stormpathApplication, query._location)
      .then((results) => {
...
router.get('/profile-and-match', stormpath.authenticationRequired, function (req, res) {
  CommuterLocations
    .findCommutersAndProfiles(req.stormpathApplication, req.query._location)
    .then((cls) => {
