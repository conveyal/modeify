...
if (config.env === 'development') {
  ...
  /**
   * TODO: refactor the package.json to do the building instead of here.
   * Until then one can run `npm run start -- no-build` to have the js not build
   * after every change made to the server code during development.
   */
...
app.all('/auth/silent-callback', function (req, res) {
  res.render('auth0-silent-callback')
})

// don't require admin, but block the rendering with auth0 lock on client if not logged in
app.all('/manager*', ensureSecure, function (req, res) {
  res.render('manager')
})

app.all('*', ensureSecure, function (req, res) {
  res.render('planner')
})
...
function ensureSecure (req, res, next) {
  if (config.env === 'development' || req.headers['x-forwarded-proto'] === 'https') {
    // OK, continue
    return next()
  }
...
function setLocals (req, res, next) {
  ...
  res.locals.google_analytics_key = config.google_analytics_key
